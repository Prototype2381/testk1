...
var GET_ONE = function(req, res) {
	console.log('Event_S 모듈 안에 있는 Get_One 호출됨.'); 

    // 데이터베이스 객체 참조
	var database = req.app.get('database');
	var _id = req.params.id;

	database.EventModel.FindOne_Schema(_id,function(results) {
		res.json(results);
		res.end();
	})
};
...
var UPDATE = function(req, res) {
	console.log('Event_S 모듈 안에 있는 UPDATE 호출됨.');
	...
	var database = req.app.get('database');
	var _id = req.params.id;
	database.EventModel.UpdateOne_Schema(_id,paramTitle,paramContents,function(results) {
		console.log("UPDATE in Event_S");
		res.json(results);
		res.end();
	})
};
...
var CREATE = function(req, res) {
	console.log('Event_S 모듈 안에 있는 CREATE 호출됨.');
	...
	var event_ins = new database.EventModel(eventdata);

	event_ins.save(function(err,result) {
...
var DELETE = function(req, res) {
	...
	database.EventModel.remove({ _id: id  },function(result){
...
var GET_LIST = function(req, res) {
	console.log('Event_S 모듈 안에 있는 GET_LIST 호출됨.');
	var database = req.app.get('database');
	...
	if(filter === "{}"){
		database.EventModel.findAll().sort(name).exec(
		...
		database.EventModel.find({name:JSON.parse(filter).search}).sort(name).exec(
			function(err, results){
				paging(results,pageIndex, perPage);
		});
	}
...
var GET_EVENTLIST = function(req, res) {
	console.log('Event_S 모듈 안에 있는 GET_LIST 호출됨.');
	var database = req.app.get('database');
	database.EventModel.findAll().sort({updated_at:-1}).exec(
		function(err, results){
			res.json(results);
		}
	);
...
var POST_EVENT = function(req, res) {
	console.log('Event_S 모듈 안에 있는 POST_EVENT 호출됨.');
...
module.exports.GET_EVENTLIST = GET_EVENTLIST;
